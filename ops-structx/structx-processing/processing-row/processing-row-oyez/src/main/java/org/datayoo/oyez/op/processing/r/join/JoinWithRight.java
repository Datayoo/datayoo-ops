...
@OpDefiner(name = "JoinWithRight",
    ...
    parameters = "",
    compoxes = {})
public class JoinWithRight extends BaseSetOperator {
  ...
  PlRowSet rightPlRowSet = null;

  public JoinWithRight(FlowNodeMetadata operatorMetadata, FlowNode parent,
  ...
  protected void innerOperate() {
    ...
    if (rightPlRowSet == null) {
      rightPlRowSet = readAll(rightInput);
    } else {
      PlRowSet plRowSet = readAll(rightInput);
      if (null != plRowSet) {
        rightPlRowSet = plRowSet;
      }
    }
    PlRowSet leftPlRowSet = this.leftInput.read();
    //需要处理
    if (leftPlRowSet == null) {
      return;
    }
    List<Object[]> leftRows = leftPlRowSet.getRows();
    ...
    //join使用左侧输入的水位线
    outputPort.write(outSet, this.leftInput.getWaterMark());
  }
  ...
  protected String buildSetSql(ColumnSetMetadata leftColumnSetMetadata,
    ...
    sb.append("left").append(" join ROW2 rightRow").append(" on ");
