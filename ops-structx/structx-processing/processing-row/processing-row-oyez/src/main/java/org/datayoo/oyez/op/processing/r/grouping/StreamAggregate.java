...
@OpDefiner(name = "StreamAggregate",
    ...
    parameters = "",
    compoxes = {})
public class StreamAggregate extends BaseProcessOperator {
  ...
  private ColumnSetMetadata columnSetMetadata;

  public StreamAggregate(FlowNodeMetadata operatorMetadata, FlowNode parent,
  ...
  protected void innerOperate() {
    ...
    String tempWaterMark = this.inputPort.getWaterMark();
    if (StringUtils.isEmpty(waterMark)) {
      waterMark = tempWaterMark;
      init(rowSet);
    }
    if (ObjectUtils.equals(waterMark, tempWaterMark)) {
      DataSetMap dataSetMap = new DataSetMapImpl();
      dataSetMap.putDataSet("ROWS", rowSet.getRows());
      selector.select(dataSetMap);
    } else {
      PlRowSet outSet = createRowSet(
          this.createOutputColumnSetMetadata(rowSet.getColumnSetMetadata()));
      outSet.addRows(selector.getRecordSet().getRecords());
      outputPort.write(outSet, this.inputPort.getWaterMark());
      this.selector.clear();
      DataSetMap dataSetMap = new DataSetMapImpl();
      dataSetMap.putDataSet("ROWS", rowSet.getRows());
      this.selector.select(dataSetMap);
      waterMark = tempWaterMark;
    }
  }

  private String buildSql(ColumnSetMetadata columnSetMetadata) {
    ...
    for (int i = 0; i < columnInfos.length; i++) {
      ...
      if (!"".equals(payloads[1])) {
        ...
        } else if (payloads[0].equals(
            AggregateDescriptor.PARAM_JOINT_BY_COMMA)) {
          sb.append("joint(row[").append(columnInfo.getColumnIndex())
              .append("],',') ").append(payloads[1]);
        } else {
  ...
  protected void init(PlRowSet rowSet) {
    columnSetMetadata = rowSet.getColumnSetMetadata();
    String sql = buildSql(rowSet.getColumnSetMetadata());
    try {
      selector = MoqlEngine.createSelector(sql);
    } catch (MoqlException var5) {
      MoqlException e = var5;
      e.printStackTrace();
    }
  }
  ...
  protected void finishOperate(RunningState state, String errorMessage) {
    if (errorMessage == null) {
      if (columnSetMetadata != null) {
        ...
        if (selector.getRecordSet().getRecordsCount() > 0) {
          outSet.addRows(selector.getRecordSet().getRecords());
