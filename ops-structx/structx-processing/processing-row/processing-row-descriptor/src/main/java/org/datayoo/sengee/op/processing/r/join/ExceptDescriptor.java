 ...
 * @author hhn
 */
@OpDefiner(name = "Except",
    type = OperatorProfileConstants.OC_PROCESS,
    version = "1.0",
    computionFramework = "sengee",
    tags = {
        @TagPair(name = SengeeConstants.TAG_OP_CATS, value = "rowset,join")
    },
    ...
    compoxes = {})
public class ExceptDescriptor extends AbstractProcessingDescriptor implements
    CompliationCsmDependent {

  public ExceptDescriptor(FlowNodeMetadata flowNodeMetadata, FlowNode parent,
  ...
  protected ColumnSetMetadata compileCsmOfOutputPort(
    ...
    ColumnSetMetadata leftMetadata = new GeneralColumnSetMetadata(this.columnSetMetadataMap
        .get(SengeeOperatorConstants.PORT_LEFT_IN));
    return leftMetadata;
  }
  ...
  protected void validateParameters() {
    ColumnSetMetadata left = this.columnSetMetadataMap
        .get(SengeeOperatorConstants.PORT_LEFT_IN);
    ColumnSetMetadata right = this.columnSetMetadataMap
        .get(SengeeOperatorConstants.PORT_RIGHT_IN);
    if (null != left && null != right) {
      ...
      if (leftColumns.size() > rightColumns.size()) {
        throw new OperationRuntimeException(OperatorsI18nMessageResource
            .format(OperatorsExceptionConstants.ECM_OPD_DIFF_COL_NUM,
                this.alias));
      }
      int i = 0;
      for (ColumnMetadata leftMetadata : leftColumns) {
        ...
        if (!leftMetadata.getType().equals(rightMetadata.getType())) {
          throw new OperationRuntimeException(OperatorsI18nMessageResource
              .format(OperatorsExceptionConstants.ECM_OPD_UNMATCHED_DATATYPE,
