...
@OpDefiner(name = "BloomFilter",
    ...
    parameters =
        "<parameters><parametergroup name=\"general\" c_Alias=\"一般\" c_Compox=\"sightx-tab\">"
            + "<parameter name=\"srcColumn\" c_Option=\"false\" c_Alias=\"数据集\" c_Compox=\"sengee-resIn:all-column-selector\"></parameter>"
            + "<parameter name=\"column\" c_Option=\"false\" c_Alias=\"待匹配数据\" c_Compox=\"sengee-dataIn:all-column-selector\"></parameter>"
    ...
    compoxes = {})
public class BloomFilterDescriptor extends AbstractProcessingDescriptor {
  ...
  public BloomFilterDescriptor(FlowNodeMetadata flowNodeMetadata,
  ...
  public void validateParameters() {
    ...
    ColumnSetMetadata dstMetadata = getPortColumnSetMetadata(
        SengeeOperatorConstants.PORT_DATA_IN);
    DataType srcType = srcMetadata.getColumn(
        this.parameters.getParameterValue(PARAM_SRC_COLUMN)).getType();
    ...
    DataType dstType = dstMetadata.getColumn(
        this.parameters.getParameterValue(PARAM_COLUMN)).getType();
